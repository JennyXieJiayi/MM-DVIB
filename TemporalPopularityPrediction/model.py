...
class Decoder(nn.Module):
	def __init__(self, emb_size, hid_size, dec_obj, dec_type='rnn', drop_p=0.2, num_t=24, num_layers=1):
		...
		if emb_size is not None:
			self.t_emb = nn.Embedding(num_t, emb_size)
		else:
			self.t_emb = None
			emb_size = 1
		...
		if dec_type == 'rnn':
			self.linear_h0 = nn.Linear(hid_size, hid_size)
			self.rnn = nn.RNN(emb_size, hid_size, num_layers=num_layers, batch_first=True)
		elif dec_type == 'lstm':
			self.lstm = nn.LSTM(emb_size, hid_size, num_layers=num_layers, batch_first=True)
			self.linear_h0 = nn.Linear(hid_size, hid_size)
			self.linear_c0 = nn.Linear(hid_size, hid_size)
		elif dec_type == 'attn':
			self.dropout = nn.Dropout(drop_p)
			self.pos_kv = PositionalEncoding(hid_size, drop_p)
			self.linear_k = nn.Linear(hid_size, emb_size)
			self.linear_v = nn.Linear(hid_size, emb_size)
			self.pos_q = PositionalEncoding(emb_size, drop_p)
			self.linear_q = nn.Linear(emb_size, emb_size)
		self.linear_dec = nn.Linear(emb_size, 1)
		self.conv1d_dec = nn.Conv1d(hid_size, 1, kernel_size=1)
		self.dec_obj = dec_obj
		if dec_obj == 'video':
			self.uv_param = nn.Parameter(torch.tensor(0.5))

	def forward(self, pop_time, sampled_zs):
		if self.dec_obj == 'video':
			sampled_z = self.uv_param * sampled_zs[0] + (1 - self.uv_param) * sampled_zs[1]
		else:
			sampled_z = sampled_zs
		pop_time = self.t_emb(pop_time.long()) if self.t_emb else pop_time.view(pop_time.size(0), -1, 1)
		if self.dec_type == 'rnn':
			h0 = self.linear_h0(sampled_z).view(1, pop_time.size(0), -1).repeat(self.num_layers, 1, 1)
			out, _ = self.rnn(pop_time, h0)
		elif self.dec_type == 'lstm':
			h0 = self.linear_h0(sampled_z).view(1, pop_time.size(0), -1).repeat(self.num_layers, 1, 1)
			c0 = self.linear_c0(sampled_z).view(1, pop_time.size(0), -1).repeat(self.num_layers, 1, 1)
			out, _ = self.lstm(pop_time, (h0, c0))
		elif self.dec_type == 'attn':
			sampled_z = sampled_z.view(pop_time.size(0), 1, -1).repeat(1, pop_time.size(1), 1)
			key = self.linear_k(self.pos_kv(sampled_z))
...
class DMMVED(nn.Module):
	...
	def forward(self, u_feat, v_feat, pop_time):
		...
		out_mean_pop = self.u_decoder(pop_time, sampled_z_u)
		v_mean, v_std = self.v_encoder(v_feat, sampled_z_u)
		sampled_z_v = self.v_sampler(v_mean, v_std)
		out_pop = self.decoder(pop_time, (sampled_z_v, sampled_z_u))
	...
	def loss(self, out_mean_pop, out_pop, tgt_mean_pop, tgt_pop, lambd_u=0.3, lambd_v=0.3, factor_uv=0.5):
